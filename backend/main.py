...
def create_source(req: SourceCreateRequest):
    ...
    return SourceResponse(source_id=req.source_id, type=req.type, status=req.status)
...
def update_source(source_id: str, req: SourceUpdateRequest):
    ...
    return SourceResponse(source_id=source_id, type=source["type"], status=source["status"])
...
def delete_source(source_id: str):
    ...
    for pipeline in PIPELINES.values():
        if source_id in pipeline["sources"]:
            raise HTTPException(status_code=400, detail="Source is used in pipelines and cannot be deleted")
